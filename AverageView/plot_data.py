...
class PlotData(QtCore.QThread):
    ...
    def plot_bokeh_stick(self,
        ...
        # Find the min and max value for the bin depth
        # Create a height based off the bin size
        bin_depth_min = df_mag.bin_depth.min()
        bin_depth_max = df_mag.bin_depth.max()
        height = ((bin_depth_max - bin_depth_min) / num_bins) * self.scale_factor_remove_gridline
        ...
        x0 = df_mag.last_ens_num.values     # Ensemble Number
        y0 = df_mag.bin_depth.values        # Bin Depth
        mag = df_mag.value.values           # Water Magnitude (Velocity)
        dir = df_dir.value.values           # Water Direction
        x1 = x0 + mag * np.cos(dir)         # Calculate the second point location based off magnitude and direction
        y1 = y0 + mag * np.sin(dir)         # Calculate the second point location based off magnitude and direction

        # Color Map
        cm = np.array(["#C7E9B4", "#7FCDBB", "#41B6C4", "#1D91C0", "#225EA8", "#0C2C84"])

        # Generate a color value for each mag value
        ix = ((mag - mag.min()) / (mag.max() - mag.min()) * 5).astype('int')
        colors = cm[ix]

        # Create a mapping between min and max value and the colors
        mapper = LinearColorMapper(palette=cm, low=mag.min(), high=mag.max())

        # Create a data source
        source = ColumnDataSource(data=dict(x=x0, y=y0, mag=mag, dir=dir, colors=colors))

        # Plot the data
        p.ray(x=x0, y=y0, length=mag, angle=dir, angle_units="deg", color=colors, line_width=2)
        #p.ray(source=source, x='x', y='y', length='mag', angle='dir', color='colors', line_width=2)
    ...
    def plot_bokeh(self, avg_result):
        ...
        output_file(self.earth_plot_file_name, "Plots: " + str(ss_code) + "_" + str(ss_config))
        ...
        stick_plot = self.plot_bokeh_stick(df_mag, df_dir, min_vel, max_vel, bt_range, ens_time_sec, num_bins, is_upward, "Water Vector", "m/s")
        ...
        save(stick_plot, filename=stick_plot_file_name, title=stick_plot_file_name)
